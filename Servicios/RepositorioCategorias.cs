...
namespace ManejoPresupuestos.Servicios
{
    ...
    public class RepositorioCategorias : IRepositorioCategoria
    {
        ...
        public async Task<IEnumerable<Categoria>> Obtener(int usuarioId,PaginacionViewModel paginacion)
        {
            ...
            return await connection.QueryAsync<Categoria>(@$"SELECT * FROM Categorias 
                                                            ...
                                                            ORDER BY Nombre
                                                            OFFSET {paginacion.RecordsASaltar} ROWS FETCH NEXT {paginacion.RecordsPorPagina} ROWS ONLY", new { UsuarioId = usuarioId }); 
        }
        public async Task<int> Contar(int usuarioId)
        {
            using var conection=new SqlConnection(connectionString);
            return await conection.ExecuteScalarAsync<int>("SELECT COUNT(*) FROM Categorias WHERE UsuarioId=@usuarioId", new { usuarioId });
        }
        ...
        {
            using var connection = new SqlConnection(connectionString);
            await connection.ExecuteAsync("DELETE Categorias WHERE Id = @Id", new { Id = id });
        }
    } 
}
