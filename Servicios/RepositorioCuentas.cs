    ...
    public class RepositorioCuentas:IRepositorioCuentas
    {
        ...
        public async Task<IEnumerable<Cuenta>> Buscar(int usuarioId)
        {
            ...
            return await connection.QueryAsync<Cuenta>($@"SELECT Cuentas.Id, Cuentas.Nombre,Balance, tc.Nombre as TipoCuenta FROM Cuentas 
                                                        INNER JOIN
                                                        TiposCuentas tc on tc.Id=Cuentas.TipoCuentaId
        ...
        {
            using var connection = new SqlConnection(connectionString);
            return await connection.QuerySingleOrDefaultAsync<Cuenta>($@"SELECT Cuentas.Id, Cuentas.Nombre,Balance,Descripcion, TipoCuentaId
                                                        FROM Cuentas 
                                                        INNER JOIN
                                                        TiposCuentas tc on tc.Id=Cuentas.TipoCuentaId
        ...
        {
            using var connection = new SqlConnection(connectionString);
            await connection.ExecuteAsync(@$"UPDATE Cuentas SET Nombre=@Nombre, Descripcion=@Descripcion, Balance=@Balance, TipoCuentaId=@TipoCuentaId WHERE Id=@Id", cuenta);
        }
